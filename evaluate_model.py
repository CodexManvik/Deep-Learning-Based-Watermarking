    ...
    if mode == "paper":
        ...
        if no_attack_ber is not None:
            comparison_table.append([
                ...
                f"{no_attack_ber:.4f}",
                "< 0.001 (Paper: 0.0003)",
                "PASS" if no_attack_ber < 0.001 else "FAIL"
            ])
        ...
        if gauss_ber is not None:
            comparison_table.append([
                ...
                f"{gauss_ber:.2f}",
                "< 15 (Paper: 7.54)",
                "PASS" if gauss_ber < 15 else "FAIL"
            ])
        ...
        if dropout_ber is not None:
            comparison_table.append([
                ...
                f"{dropout_ber:.2f}",
                "< 20 (Paper: 11.71)",
                "PASS" if dropout_ber < 20 else "FAIL"
            ])
...
if __name__ == "__main__":
    ...
    if save_samples:
        print(f"\n[OK] Visual samples saved to: {SAMPLE_OUTPUT_DIR}")
